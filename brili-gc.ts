...
export class Heap<X> {
  ...
  private readonly storage: Map<number, [X[], number]> //number is reference count
  constructor() {
  ...
  private freeKey(key: Key) {
    // TOOD: check if the value it points to is another pointer, and if so decRefCount of that pointer 
    let mapping = this.storage.get(key.base) as [X[], number];
    let array = mapping[0]
    // check if array is type pointer. 
    if (array[0] && typeof array[0] === 'object' && 'loc' in array[0] && 'type' in array[0]) {
      for (const value of array) {
        // for each of the elements, if it's defined then call decRefCount on each of them 
        if (value && value !== undefined) {
          this.decRefCount((value as unknown as Pointer).loc);
        }
      }
    }
  ...
  free(key: Key) {
    //DONE: get rid of key.offset == 0 condition
  ...
  write(key: Key, val: X) {
    let data = this.storage.get(key.base)[0];
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    //DONE: must increment the refcount of all arg pointers
    if (value && typeof value === 'object' && 'loc' in value && 'type' in value) {
      let loc = (value as Pointer).loc;
      state.heap.incRefCount(loc);
    }
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  switch (instr.op) {
    ...
    case "id": {
      ...
      // DONE: check if val is a pointer and if so increment its ref count 
      // if dest points to something then decrement its ref count 
      if (val && typeof val === 'object' && 'loc' in val && 'type' in val) {
        let loc = (val as Pointer).loc;
        state.heap.incRefCount(loc);
        if (state.env.has(instr.dest)) {
          let val = state.env.get(instr.dest);
          let loc = (val as Pointer).loc;
          state.heap.decRefCount(loc);
        }
      }
    ...
    case "ret": {
      ...
      if (args.length == 0) {
        ...
        //DONE: increment ref count if the return value is a pointer
        if (val && typeof val === 'object' && 'loc' in val && 'type' in val) {
          let loc = (val as Pointer).loc;
          state.heap.incRefCount(loc);
        }
    ...
    case "alloc": {
      ...
      //DONE: if dest currenly points to loc in the env, then 
      //decrement ptr refcount here 
    ...
    case "ptradd": {
      ...
      //DONE: incr ref count of new pointer add 
      //DONE: if ptr pointed to something then dec its ref count 
      //important that it happens in this order for the case of same mem location
      state.heap.incRefCount(ptr.loc);
      if (state.env.has(instr.dest)) {
        let val = state.env.get(instr.dest);
        let loc = (val as Pointer).loc;
        state.heap.decRefCount(loc);
...
function evalFunc(func: bril.Function, state: State): Value | null {
    ...
    if ('op' in line) {
      ...
      switch (action.action) {
        case 'end': {
          ...
          //DONE: go through state.env and decrement ref count of all pointers
  ...
  //We have reached the end of the function without calling return 
  //DONE: go through all local variables (el of heap) and increment ref count of all pointers
  for (const value of state.env.values()) {
    // check if pointer
    if (value && typeof value === 'object' && 'loc' in value && 'type' in value) {
      let loc = (value as Pointer).loc;
...
function evalProg(prog: bril.Program) {
  ...
  if (!heap.isEmpty()) {
    throw error(`Some memory locations have not been freed by end of execution. You have a memory leak!!`);
  }
...
/* Garbage Collection:
  * Add a new field in storage that is a ref count
  * upon alloc, set it to 1
  * if pointer that pointed to it is overwritten with another alloc or ptradd, decrement ref count
       * another way to say this is if a pointer is on the LHS, decrement ref count
       * Make sure to do this last because it might briefly be 0. Alternatively, check for 0 after whole line has run 
  * if a pointer is on the RHS (such as ptradd or just assigning pointer)
        * increment that pointer ref count
  * when a variable goes out of scope (i.e. we return from a function), decrement all the ref counts of variables
  * make sure when it's passed as an argument, the ref count increases because now arg points to it
  * when freeing, make sure to recursively update the ref count if it points to a pointer
*/

/*
  Testing:
    * use my own custom examples to get basics right 
    * use brench to run it on all the benchmarks in mem with all the frees deleted (use awk or something to remove) 
    * compare with normal brili and the frees still there
    
    
    * Make sure correct behavior with arguments

  test if I can have a pointer to a pointer (would mean recursive frees)
  see what happens if I do an id with the wrong type
